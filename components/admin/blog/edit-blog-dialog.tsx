...
export function EditBlogDialog({
  ...
  useEffect(() => {
    if (fullBlog && open) {
      // Handle pre-migration docs that may still have legacy field names
      const legacy = fullBlog as any;
      form.reset({
        ...
        content_ro: fullBlog.content_ro ?? legacy.content ?? "",
        content_en: fullBlog.content_en ?? legacy.content ?? "",
        status: fullBlog.status,
        tags: fullBlog.tags || [],
        readingTime_ro: fullBlog.readingTime_ro ?? legacy.readingTime ?? 0,
        readingTime_en: fullBlog.readingTime_en ?? legacy.readingTime ?? 0,
      });
