...
import { PricingDetails, RentalDetails } from "../types";
import {
...
interface PricingSectionProps {
  ...
  labels?: {
    heading?: string;
    rentalLine?: (days: number, pricePerDay: number) => string;
    promoCode?: string;
  ...
  return (
    <Section className="px-[16px] py-[12px]">
      ...
      {pricingDetails.isSCDWSelected &&
        (pricingDetails.protectionCost || 0) > 0 && (
          <Row className="mb-[8px]">
            <Column className="w-2/3">
              <Text className="text-[16px] text-gray-800 m-0">
                {labels?.scdwText ?? "SCDW (zero deductible)"}:
              </Text>
            </Column>
            <Column className="w-1/3 text-right">
              <Text className="text-[16px] text-gray-800 m-0">
      ...
      {!pricingDetails.isSCDWSelected &&
        pricingDetails.deductibleAmount !== undefined && (
          <Row className="mb-[12px]">
            <Column>
              <Text className="text-[14px] text-gray-800 m-0">
                {(labels?.warrantyText ?? "Warranty (deductible)") + ": "}{" "}
                {formatCurrency(pricingDetails.deductibleAmount || 0)}
              </Text>
            </Column>
          </Row>
        )}
