...
/** Pending reservations and transfers, newest first. */
export const getPendingApprovals = query({
  args: { limit: v.optional(v.number()) },
  returns: v.array(overviewItemValidator),
  handler: async (ctx, args) => {
    await requireAdmin(ctx);
    const limit = Math.min(args.limit ?? DEFAULT_LIMIT, MAX_LIMIT);
    ...
    const items = await Promise.all([
      ...reservations.map((reservation) =>
        toReservationItem(ctx, reservation, reservation._creationTime),
      ),
      ...transfers.map((transfer) =>
        toTransferItem(ctx, transfer, transfer._creationTime),
      ),
    ]);
...
/** Latest bookings of either kind by creation time. */
export const getRecentActivity = query({
  ...
  handler: async (ctx, args) => {
    ...
    const [reservations, transfers] = await Promise.all([
      ctx.db.query("reservations").order("desc").take(limit),
      ctx.db.query("transfers").order("desc").take(limit),
    ]);
